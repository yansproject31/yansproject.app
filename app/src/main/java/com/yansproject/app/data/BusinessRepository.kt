...
class BusinessRepository(private val db: AppDatabase) {
        ...
        for (item in items) {
            ...
            if (parsed != null) {
                ...
                val varian = variants.find { 
                    ...
                    (parsed.varianName.isNotBlank() && (
                        it.nama_warna.trim().equals(parsed.varianName.trim(), ignoreCase = true) ||
                        it.nama_warna.contains(parsed.varianName, ignoreCase = true) ||
                        parsed.varianName.contains(it.nama_warna, ignoreCase = true)
                    ))
                } ?: variants.find { 
                    (catalog != null && it.id_catalog == catalog.id_catalog) &&
                    (parsed.catalogName.contains(it.nama_warna, ignoreCase = true))
                } ?: if (catalog != null) {
                    variants.firstOrNull { it.id_catalog == catalog.id_catalog }
                } else null
            ...
            if (isFullRefund) {
                ...
                invoice.projectId?.let { pId ->
                    projectDao.getProjectById(pId)?.let { project ->
                        ...
                        projectDao.updateProject(updatedProject)
                        FirebaseSyncManager.syncItemToCloud("projects", project.id.toString(), updatedProject)
                    }
                }
                ...
                val syncKey = refundedInvoice.invoiceNumber.ifEmpty { refundedInvoice.id.toString() }
                FirebaseSyncManager.syncItemToCloud("invoices", syncKey, refundedInvoice)
            } else {
                            ...
                            if (parsed != null) {
                                ...
                                if (catalog != null && varian != null) {
                                    ...
                                    if (masterStock != null) {
                                        ...
                                        db.stockHistoryDao().insertHistory(historyEntry)
                                        val docId = "${now}_${parsed.size}_${parsed.sleeve}"
                                        FirebaseSyncManager.syncItemToCloud("stock_history", docId, historyEntry)
                ...
                val syncKey = updatedInvoice.invoiceNumber.ifEmpty { updatedInvoice.id.toString() }
                FirebaseSyncManager.syncItemToCloud("invoices", syncKey, updatedInvoice)
            }
    ...
    companion object {
        fun parseInvoiceItemDetails(description: String): ParsedItem? {
            ...
            if (parts.size >= 4) {
                ...
                val isSleevePart2 = part2.contains("Panjang", ignoreCase = true) || part2.contains("Pendek", ignoreCase = true)
                if (isSleevePart2) {
                    val slv = if (part2.contains("Panjang", ignoreCase = true)) "Panjang" else "Pendek"
                    val rawSize = parts[1].trim()
                    val part0 = parts[0].trim()
                    val (cat, varName) = if (part0.contains("(") && part0.contains(")")) {
                        Pair(part0.substringBefore("(").trim(), part0.substringAfter("(").substringBefore(")").trim())
                    } else {
                        Pair(part0, "")
                    }
                    return ParsedItem(
                ...
                val isSleevePart1 = part1.contains("Panjang", ignoreCase = true) || part1.contains("Pendek", ignoreCase = true)
                val knownSizes = listOf("XS", "S", "M", "L", "XL", "XXL", "3XL", "4XL", "ALL SIZE")
                val isSizePart1 = knownSizes.any { part1.uppercase().startsWith(it) } || part1.startsWith("KIDS", ignoreCase = true)
                if (isSizePart1 || isSleevePart1) {
                    val slv = if (part1.contains("Panjang", ignoreCase = true)) "Panjang" else "Pendek"
                    val cleanSize = part1.replace("Lengan Panjang", "", ignoreCase = true)
                        .replace("Lengan Pendek", "", ignoreCase = true)
                        .replace("Panjang", "", ignoreCase = true)
                        .replace("Pendek", "", ignoreCase = true)
                        .trim()
                    ...
                    return ParsedItem(
                        catalogName = cat,
                        varianName = varName,
                        size = if (cleanSize.isNotBlank()) cleanSize else "All Size",
                        sleeve = slv
                    )
                } else {
                    return ParsedItem(
    ...
    private fun updateStockQtyForSizeSleeve(stock: MasterStock, size: String, sleeve: String, delta: Int): MasterStock {
        ...
        val normalizedSize = when (size.uppercase().replace("_", "").replace(" ", "").trim()) {
            "2XL" -> "XXL"
            "3XL", "XXXL" -> "3XL"
            "4XL", "XXXXL" -> "4XL"
            else -> size.uppercase().replace("_", "").replace(" ", "").trim()
        }
        return when (normalizedSize) {
    ...
    suspend fun updateInventorySummaryForVarian(idVarian: Int) {
        ...
        val approvedStatuses = listOf("DISETUJUI", "LUNAS", "DP", "DP AWAL", "DP PRODUKSI", "BELUM LUNAS", "COMPLETED", "PAID", "PARTIAL_REFUND")
    ...
    suspend fun restoreInvoice(invoice: Invoice) {
        db.withTransaction {
            val restoredInvoice = invoice.copy(isDeleted = false)
            invoiceDao.updateInvoice(restoredInvoice)
            ...
            if (isDeductingStatus) {
                // If the invoice was previously restored (stock returned when deleted), re-deduct physical stock upon un-deleting
                val existingLedgers = db.inventoryLedgerDao().getLedgerList()
            ...
            reconcileAllInventorySummaries()
            val cloudKey = restoredInvoice.invoiceNumber.ifEmpty { restoredInvoice.id.toString() }
            FirebaseSyncManager.syncItemToCloud("invoices", cloudKey, restoredInvoice)
        }
    }
